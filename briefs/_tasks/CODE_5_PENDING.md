...
**To:** Code Brisen #5 (fresh clone `~/bm-b5`, HEAD after PR #48 merge)
**Task posted:** 2026-04-23
**Status:** OPEN — `AUTHOR_DIRECTOR_GUARD_1` (CHANDA detector #4 pre-commit hook + §7 amendment-log entry)

**First task for b5** — mailbox previously unused. Clone is at `~/bm-b5`, latest main.
...
Ships CHANDA detector #4 per Research Agent's 2026-04-21 engineering-matrix sequencing (Step 2 of 4 — ENFORCEMENT_1 already shipped as PR #45). Bundles a 1-line amendment-log entry in `CHANDA_enforcement.md` §7 per Director's "bank-client" framing — decide + ship, don't pester on minor edits.
...
- **Ship report:** `briefs/_reports/B5_author_director_guard_1_20260423.md`. Include all 9 Quality Checkpoint outputs + 1-line CHANDA_enforcement.md diff + `git ls-files --stage` line showing executable bit.
...
- **Do NOT** modify `scripts/check_singletons.sh` — unrelated singleton hook; two scripts live independently.
...
- **Do NOT** refactor awk/grep patterns to a "clever" one-liner — readability over cleverness; mirror `check_singletons.sh` style.
...
**Working dir:** `~/bm-b5` (fresh clone, first dispatch).
