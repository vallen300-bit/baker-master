...
**D2 — Card visuals.** Drop the AG pill entirely. Card geometry: narrower + taller (portrait-leaning) — match Brisen Lab card proportions (Lab app.js/CSS is the reference register; Director: "check the sizes in production and lab, these are good sizes"). Lab-style drop shadow so cards lift from the background. Names stay one line (cowork names already de-"Desk"-ed in contract). Cowork/APP cards: SAME dimensions as terminal cards, recessed/inner-shadow treatment kept, APP marker kept.
...
**D4 — Context band (driveable Claude seats).** Thin fill-line at card bottom: context-window usage 0-100%. Data source: forge snapshot telemetry via controller (`/api/agents` gains `context_pct` where known; null → line hidden). Coordinate with deputy-codex's queued LAB_CONTEXT_BAND_EXPOSURE_1 (contract #12055) — if its endpoint is not live yet, ship the UI reading null gracefully; do NOT block the redesign on it. Approximate is acceptable (Director: "even if it is not precise").
