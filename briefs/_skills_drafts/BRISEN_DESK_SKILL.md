...
This is V1, ratified 2026-04-30 under the Manus filesystem-as-memory pattern. Refine from practice. **Slug `brisen` is added to `slugs.yml` separately as a meta-matter** (baker-vault PR #37, version 17) to keep Brief 1 path whitelist + frontmatter discipline unchanged.
...
- Foundation: BRIEF_BAKER_VAULT_WRITE_1 §3 + RA-23 + Director ratification 2026-04-30 (Brisen Desk pattern; own write surface at `wiki/matters/brisen/`; slug `brisen` added to slugs.yml as a meta-matter via baker-vault PR #37, version 17)
- **Naming history:** drafted initially as "Brisen Desk"; interim rename to "AI CEO" considered 2026-04-30, **rejected by Director EOD 2026-04-30** (cosmetic churn). Reverted to "Brisen Desk" with slug `brisen`. Aliases `brisen-desk` / `ceo` / `ai-ceo` / `brisen-ceo` all resolve to canonical `brisen` (slugs.yml v17).
- Deployment: `~/.claude/skills/brisen-desk/SKILL.md` after Briefs 1+2 ship + slug `brisen` added to slugs.yml (vault PR #37 — open at draft-time)
- Companion files (vault): `_ops/agents/brisen-desk/{OPERATING,LONGTERM,ARCHIVE}.md`
- Slugs.yml addition (shipped via baker-vault PR #37):
  ```yaml
  - slug: brisen
    status: active
    description: "Brisen Desk meta-matter — portfolio synthesizer scoped agent's own write surface. Cross-cutting; reads all matter Desks (AO / MOVIE / Hagenauer / Origination), writes only here. Synthesizes portfolio-level positions, arbitrates inter-Desk conflicts, supports Director's strategic decisions on capital allocation, portfolio reweighting, kill-criteria, and acquire-vs-walk. Does not write into other matter wikis (handoff via _inbox/ only)."
