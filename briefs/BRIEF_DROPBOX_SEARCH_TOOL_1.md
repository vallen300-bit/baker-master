...
```python
    def _resolve_path_root_header(self) -> dict:
        ...
        try:
            ...
            # DropboxPathRootError: NEW dedicated exception, define in
            # triggers/dropbox_client.py (class DropboxPathRootError(Exception))
            # -- codex G0 #12343: it does not exist in the repo yet.
...
## Files Modified
- `triggers/dropbox_client.py` — add `search()`, `_resolve_path_root_header()`, NEW `class DropboxPathRootError(Exception)` (fail-closed resolver error, codex G0 #12343), extend `_api_post` with `extra_headers`
