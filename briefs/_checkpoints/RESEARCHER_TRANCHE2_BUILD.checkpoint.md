...
- **NEXT CONCRETE STEP:** await LEAD design-review verdict. On PASS → build in a fresh baker-vault worktree on branch `b1/researcher-tranche2-item7-recency` (worktree pattern per Build-mechanics note above) → LEAD+deputy review → lead merge. On DESIGN-CHANGES → fold in design doc, re-post to lead.
