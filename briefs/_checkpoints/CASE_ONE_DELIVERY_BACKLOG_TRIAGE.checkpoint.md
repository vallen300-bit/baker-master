...
- **AUTHORING QUEUE (lead #11158) — ✅ BOTH DRAFTED + POSTED TO LEAD #11200 @b39e5c7 (2026-07-14 ~11:36Z), awaiting lead line-read:**
  1. **FLEET_DEPLOY_PARITY_1** ✅ authored + ✅ AMENDED @e01ef9d (lead #11248 → notified #11249). Leg A host-side: `--check` repo↔deployed sha256 parity (gap = `install_arm_alarm_job.sh:55-58` proves parse not match) + `arm_fleet_manifest.json` + `arm_fleet_parity.sh` (missing=RED) + F3 db_unreachable/DEGRADED→RED-with-names (findings 1/2/8) + F4 MISSING_IS_RED ladder flip (finding-4). **Leg B / F5 (folded #11248):** `fleet_client_parity.sh` — per-seat `bus_post.sh` sha-vs-origin + started-emit capability probe + dispatcher stale-seat roll-up (unconfirmable=RED); BINDING rider: client sweep + all seats CLEAN BEFORE arming-ladder step-4 drain measurement.
  ...
  3. **DEFAULT_FLIGHT_INFRA_FILTER_1** ✅ authored @c1ec32e → posted lead #11257 (awaiting review). `orchestrator/airport_ticketing_bridge.py`: **Fix 1** `_is_infra_ops_mail` upstream of mint (:903) — self-originated (Brisen OUTBOUND sender) + infra-subject (real ARM subjects arm_alarm_check.sh:321/345/373 + watchdog/canary/fire-drill, env-extensible) → drop-log; **Fix 2** `_REVIEW_FLIGHT="unrouted-review"` replaces `_flight_name()` on review branch (:940) so un-corroborated arrival never inherits aukera-annaberg flight. Defense-in-depth: #11218 stops under either fix. Deferred-flag: global `_DEFAULT_FLIGHT` neutralize = separate fleet-wide change (AIRPORT_DEFAULT_FLIGHT_NEUTRALIZE_1).
- **✅ AUTHORING QUEUE COMPLETE (3/3).** Lead #11250 reviewed #1+2 = BOTH PASS as written. DISPATCH: FLEET_DEPLOY_PARITY_1 → deputy-codex builds (gate b4); RECEIPT_WRITE_DURABILITY_1 → b1 builds (gate codex). Brief #3 (#11257) awaiting lead review. **deputy = live-AC/bus-health owner for all 3 post-deploy.** Ladder: step-4 drain counts only after F5 client-parity sweep all-CLEAN (#11248). ⚠️ Watch: `a089d90` #560 ARM semantic ENFORCE gate — adjacent to FLEET_DEPLOY F3/F4; flagged, confirm no overlap.
