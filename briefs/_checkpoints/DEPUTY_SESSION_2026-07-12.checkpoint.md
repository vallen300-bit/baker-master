...
- **NEW (lead #9419, queued):** ARM cage gaps from codex-arch #9417 — author single-PR brief: (1) secrets-read-deny Read-path hook (mirror librarian) -> deputy-codex/b4, codex gate + lead merge; (2) check SPEC_ARM v1.2 for arm_sql.sh, fold or N/A; (3) reconcile install record (v1.2 is DRAFT-until-ramp; ratified thing was Flight Academy install #9217, not v1.2).
  - **AMEND #9423:** Director made ARM a NORMAL agent (codex-arch #9421 — cages removed, external-send + Baker-mutation denials kept). So #9419 items (1) secrets-read-deny + (2) arm_sql.sh are likely OBE — VERIFY against new ARM posture BEFORE briefing (do not build dead work). Item (3) record-fix STANDS. Also verify canonical _ops/agents/arm/* changes are committed.
