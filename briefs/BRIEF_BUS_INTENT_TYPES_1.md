...
### 2 — ARM unacked-SLO keys on `intent==command`, refined by the EXISTING P5 predicate (NO new gate)
ARM's unacked-SLO alarm (the charter predicate that releases HOLD #10422) fires on:

```
intent == "command"  AND  _is_delivery_tracked(kind, execute_obligation)  AND  unacked_past_SLA
```

`_is_delivery_tracked` is **reused verbatim** (`bus.py:88`) — it already resolves to the root worker-start dispatch and already **excludes** `ratify_required` (a Director-gated decision path, not a worker-start dispatch — the existing P5 comment). So:
- `intent==command` is the **charter-nameable label** ARM arms on (satisfies v1.1a F4 "machine-readable dispatch state" + releases #10422).
- `_is_delivery_tracked` is the **operational refinement** of which commands carry the *worker* unacked-SLO — unchanged, so P5's scope does not move.
- `ratify_required` is correctly a *command* (obligation-bearing) but rides the Director-gated ratify lane, NOT the worker unacked-SLO — the label and the SLO-refinement being separate is exactly what keeps this clean.

This adds **no new firing condition** and **no second warning** — it renames/canonicalizes the predicate P5 already computes, so ARM has one field to arm on.
...
- Tests (brisen-lab): (a) derivation table — each of the 6 kinds maps to the correct intent; (b) server ignores/overrides a client-supplied `intent` mismatching the kind; (c) backfill sets every legacy row; (d) **no-double-gate** — a root dispatch trips `_is_assignment` exactly once and is labeled `command`; a `ratify_required` is `command` but is NOT delivery-tracked and does NOT trip the worker unacked-SLO; a `reply` (parent_id set) is `command`? no — a reply is `kind=dispatch`+parent → still `command` by kind but NOT `_is_assignment` and NOT warned (confirm the reply case explicitly); an `ack`/`broadcast`/`alert` is `event` and trips nothing; (e) ARM SLO predicate fires on an unacked past-SLA root command and does NOT fire on any event.

> **Reply-case note for the builder (must resolve in the test):** a threaded reply is `kind=dispatch` with `parent_id` set, so by the derivation it is labeled `intent=command`. That is acceptable (it *is* obligation-bearing by kind) and does NOT create a double-warning because `_is_assignment` excludes it (parent_id set) and `_is_delivery_tracked`… **currently keys on `kind=='dispatch'` only, so a reply IS delivery-tracked today.** Confirm with lead whether that is intended (P5 already tracks replies) — if replies should be event-class, the derivation must special-case `parent_id is not None → event`. **Flag, do not silently pick.** (Surface-conflict discipline — this is the one place the coarse `kind`-only derivation and the finer `_is_assignment` root-check disagree; the builder must not average them.)
