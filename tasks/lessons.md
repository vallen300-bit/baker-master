...
## Lesson #117 (2026-07-08, b1) — Seeded-violation suites must test the HAPPY path, not only fail-closed rejects
LIBRARIAN_AGENT_INSTALL_1 shipped a 26-test seeded suite that gated the cage across three review rounds (deputy #7469, codex #7476, delta-verify #7489). Every reply-wrapper test in it (d.1/d.2/d.3) asserted a REJECT (empty thread-id, unverifiable thread, raw bus_post denied). None asserted that a VALID inbound thread is ACCEPTED. So defect F3 — `librarian_bus_reply.sh` derived the requester with a single-quoted f-string inside a single-quoted `python3 -c '...'` block, whose inner `f'` closed bash's quote and killed the command substitution — sat invisible through all three gates and only surfaced at Part C rung-1, the librarian's first REAL reply. The bug false-rejected every legitimate ticket: the seat could never have answered anyone.
**Rule:** a seeded/violation suite that only proves "the bad thing is blocked" is half a suite. For every enforcement path, add at least one POSITIVE test proving the ALLOWED path still works end-to-end (the happy path a real user hits first). Deny-only coverage passes green while the feature is 100% broken. Fix added h.1 (static guard: no single-quoted f-string in the `-c` block) + h.2 (functional: derivation resolves the requester from a stubbed inbound mailbox) → suite 28/0. Corollary: never put a single-quoted f-string inside a single-quoted `python3 -c '...'`; use double-quote + concat.
