...
## Lesson #117 (2026-07-11, b2) — For baker-vault work use your DEDICATED worktree, not the shared `~/baker-vault`; and run UNSCOPED `git status` before every commit
While authoring ARM_CAGE_SCRIPTS_AUTHOR_1 I worked in the shared `~/baker-vault` checkout. Two failures resulted: (1) my PR-1 (baker-vault #154) swept in an out-of-scope `wiki/matters/brisen/_people.md` that was pre-staged in the shared index by another agent — I "verified" clean with a path-scoped `git status --short _ops/agents/arm/`, which by construction could not show the `wiki/` path (deputy caught it, #8918); (2) mid-arc, concurrent BRI-GRP commits (`d8d14a0`, `b5736fc`) thrashed my branch pointers on the shared HEAD, landing a foreign commit between my PR-1-fix and PR-2 and scrambling which branch my PR-2 commit attached to.
**Rule:**
1. Each B-code has a DEDICATED baker-vault worktree (`~/bm-b2-baker-vault`, etc.) — use it for all baker-vault edits. The shared `~/baker-vault` is a multi-agent collision zone (branch HEAD moves under you). If you must touch it, isolate via `git worktree add` off the clean pushed head before committing.
2. Before EVERY commit run an UNSCOPED `git status --short` (never path-scoped) and confirm the staged set (`git diff --cached --name-only`) equals exactly your intended files. A scoped status hides sibling paths another agent pre-staged.
3. Recovery when the shared HEAD has thrashed: don't fight it in place — `git worktree add` off `origin/<clean-branch>`, cherry-pick your own commit (its diff is your files only), push from the isolated worktree. Never reset/force a shared branch that carries another agent's un-pushed commits — flag those for their owner instead.
Extends the standing baker-vault shared-FS race lesson (dedicated worktree is the proper fix, atomic single-shell commit only a stopgap).
