...
## Lesson #116 (2026-07-08) — A blocking Stop hook can never allow its own exit
The rollover Stop hook (`context-threshold-check.sh`) returned `decision:block` on EVERY Stop over the hard band. In Claude Code, a Stop hook that blocks forces the session to CONTINUE, not stop — so it can structurally never permit the "exit cleanly" it was ordering. Worse, each blocked turn appended to the transcript, and since `percent = bytes/4 / window`, the number climbed every cycle — a self-feeding loop. The Baden-Baden desk had already done everything right on the first fire (checkpoint written+pushed, respawn posted) but got trapped: 137→153%, +21.4k tokens, ~6 min, only killed by the Director watching it (PR #489).
**Rule:** any Stop/PreToolUse hook that emits `decision:block` to force a one-time action MUST be idempotent — block AT MOST ONCE (guard on a per-session marker keyed to `transcript_path`, or equivalent), then fall through to `block=False`. A blocking hook with no exit condition is an infinite loop, not a gate. Fail-safe direction is always "allow exit" (marker-write failure → don't block), never "keep blocking." And measurement-at-Stop-only means a long single turn can blow past soft bands in one jump — that's the outer watchdog's job, not the Stop hook's.

## Lesson #117 (2026-07-11) — Stacked PRs: "MERGED" ≠ "on main" — verify base before, target after
Lead merged vault PRs #154+#155 back-to-back on a combined codex PASS; both showed state=MERGED, so lead reported "both in main". But #155's base was the PR-1 feature branch (stacked), never retargeted — its 5-file ARM cage delta merged into the feature branch, not main. ARM seat symlinks went dangling (exit 127 on all inputs; cage inert) until deputy's fail-loud (#8960) caught it; fix = fresh PR #156 from the branch to main. The reviewer's merge-order note (#8952) warned exactly this but crossed with the merge.
**Rule:** stacked-PR merge sequence is: (1) check `baseRefName` on EVERY PR before merging — retarget to main first (or merge base-first and let GitHub retarget, then RE-CHECK); (2) after merging, verify presence on the target branch itself (`git cat-file -e origin/main:<key-file>`), never trust state=MERGED; (3) a gate verdict that arrives with a MERGE ORDER section is a pre-merge checklist, not commentary — read it before pressing merge.

## Lesson #118 (2026-07-12, b3) — Settings-registered ≠ metered: a fleet coverage audit must check the SCRIPT is present, not just the hook registration
CASE_ONE_P0 P0.2 fleet audit classified a picker WIRED purely because its `.claude/settings(.local).json` registered the Stop hook. But the registered command is a RELATIVE path (`.claude/hooks/context-threshold-check.sh`); the hook is fault-tolerant and no-ops silently when the script is absent. All 13 standalone pickers (cowork App dirs, designer/publisher/clerk) had the registration path but NOT the script — a settings-only sweep would have reported 21/21 covered while metering NOTHING. The hardened audit (require registered AND `context-threshold-check.sh` + `context_meter.py` present) then also caught 3 baker-master clones — including lead's own `bm-aihead1` — that carried the new import-based hook without the new `context_meter.py` module (merged but not yet pulled), so their hooks would `import context_meter` → no-op.
**Rule:** any "is capability X wired across the fleet" audit must verify the ARTIFACT THE CONFIG POINTS AT actually exists and is loadable, not just that the config references it. Registration is a pointer; a pointer to an absent/again-unloadable target is false-green — the most dangerous audit result because it reads as done. Distribution of a hook = settings registration + script file(s) present in that picker; a shared module split out of a hook (here `context_meter.py`) becomes a second required artifact that every consumer picker/clone needs, or the importing hook dies silently. Fail-loud: name every MISSING_SCRIPT seat, exit nonzero. Extends Lesson #113 (verify via the path the consumer actually uses).

## Lesson #119 — Codex session-start bus scan must auto-execute pending dispatches (2026-07-11)
Codex reviewer read gate dispatch #9113 (PR #525 G3) at session start, reported it as status, and idled — lead had to send an explicit "EXECUTE NOW" nudge (#9115). Root cause: `~/.codex/AGENTS.md` had the execute-immediately rule only for mid-session bus pings; its session-start block said "check inbox + confirm" with no handle-it clause. Deputy-codex (`~/bm-aihead2/AGENTS.md`) already had the clause ("handling <topic> now") and was not affected. Fix: mirrored the clause into `~/.codex/AGENTS.md` session-start block (Director-ratified 2026-07-11). Rule: any seat's session-start inbox scan that finds an unacked dispatch must start executing in the same turn — a startup scan is not exempt from superior-dispatch authority.

## Lesson #120 — FF-pull local ~/baker-vault BEFORE symlinking vault-canonical artifacts (2026-07-12)
Merged vault PR #164 (arm_check_inbox.sh) then symlinked ~/bm-arm/scripts -> ~/baker-vault canonical path — but local ~/baker-vault main was 1 commit behind origin, so the symlink DANGLED until deputy FF-pulled (bus #9157). GitHub merge does not touch the shared local checkout. Rule: any post-merge wiring that targets ~/baker-vault (symlinks, cage re-sync, path references) starts with `git -C ~/baker-vault pull --ff-only origin main` and a file-exists check on the target before declaring the wiring done.
...
## Lesson #123 (2026-07-19, lead) — Infra migrations are load multipliers: re-check capacity ceilings after every fleet-topology change
The 2-day bus outage ("agents don't wake") was NOT a broken wake handler. The Terminal→tmux+ttyd cutover moved all 28 seats onto Lab-mediated glance/wake polling and stacked duplicate lifecycle-watchers (6 found for one alias, 5s interval each) — fleet load quietly crossed the hardcoded `maxconn=10` DB-pool ceiling in brisen-lab `db.py`. Everything "worked before the cutover" only because load sat below the cliff. Compounding trap: the autowake path FAILS CLOSED on DB-read errors, so saturation silently suppressed wakes with zero alerting — the safety gate turned a capacity problem into an invisible delivery problem. Fixed by pool 10→25 env-tunable (then 40), de-gated receipt writes, glance cache 60s + indexes (brisen-lab @db8e27f).
**Rule:** after any fleet-topology or infra migration, re-derive the load arithmetic against every hardcoded capacity ceiling (pool sizes, semaphores, worker counts) BEFORE calling the migration done; and every fail-closed safety gate MUST emit a loud counter (suppressed-count + first/last timestamp) — silent fail-closed is how outages hide for two days.

## Lesson #124 (2026-07-19, lead) — Diagnose-first beat the plausible hypothesis; adversarial deep-verify beat single-threaded review
Three diagnosis lessons from the same outage, all cheap to repeat: (1) The "obvious" culprit — `COUNT(*) OVER()` on every /msg read — was REFUTED by EXPLAIN ANALYZE (~1.5ms); shipping that hypothesized fix would have done nothing while the outage continued. Deputy-codex's diagnose-first discipline caught it; the real driver was pool saturation. (2) A 43-agent adversarial deep-verify (6 lenses + refuters) found what serial review missed: the duplicate-watcher load multiplier, the merged-tree gating requirement, and a latent lifecycle gate-contract P1 (`bus.py` async paths calling sync `db.*` directly). (3) Gate the MERGED tree, not the branch tip — a stale merge-base made the branch diff show last night's wake arcs as phantom deletions, and the real P1 above only surfaced in the merged-tree pass.
**Rule:** capture the EXPLAIN/repro evidence BEFORE building the fix (hypothesis ≠ diagnosis); for outage-class or multi-arc merges, run the gate on the merged tree; when one reviewer chain keeps missing, escalate to parallel adversarial verification rather than another serial pass.

## Lesson #125 (2026-07-19, lead) — When the coordination channel is the thing that's broken, keep out-of-band levers ready
The bus outage blocked the coordination needed to fix the bus (deputy couldn't read the builder's diffs; gate verdicts couldn't post; wake nudges to the reviewers were themselves suppressed). The deadlock broke only via out-of-band paths: Render API restarts (3× relief windows + env changes + deploy trigger), direct tmux pane injection with capture-pane verification, reading diffs straight from git instead of bus relays, and hand-carrying gate verdicts from the reviewer's pane. Sub-trap: tmux text injected into a busy Codex CLI seat parks UNSUBMITTED in the composer (Enter does not queue mid-turn — Tab does); a parked nudge looks sent but never fires.
**Rule:** for every critical shared channel, maintain and document at least one out-of-band control lever (API restart, direct pane inject+verify, git-direct reads) BEFORE the crisis; after any injection into a busy seat, capture-pane to confirm the message actually queued/submitted rather than parked.

## Lesson #126 (2026-07-20, lead) — When the reviewer seat dies, reroute the gate lane with provenance — never loop seat kills, never skip the gate
The codex bus seat context-exhausted mid-verdict 3× in one night (plus post-refresh resets that silently dropped injected nudges). The unblock that worked: reroute the gate to the codex-verify CLI lane (same cross-vendor model, provenance line quoted on the bus), stand the seat lane down explicitly to avoid duplicate verdicts, and keep going. 8 CLI gate rounds ran overnight this way; every verdict was real (multiple P1s caught). The two failure modes avoided: (a) waiting hours for a dead seat, (b) merging ungated "because the reviewer is down."
**Rule:** the GATE is mandatory; the LANE is fungible. On reviewer-seat failure: one recovery attempt max, then reroute to the CLI lane with provenance posted, and explicitly stand down the dead lane so duplicate verdicts can't race.

## Lesson #127 (2026-07-20, lead) — Iterative adversarial gating converges: 14 rounds, 23 real findings, zero bad merges
Tonight's three arcs (storm-fix, listener, disposition) took 3-6 gate rounds EACH before clean PASS. Every round's findings were genuine production hazards: fail-open wake row selection, SSE head-of-line blocking, wake-loss on pool saturation, silent feature-kill via an unrelated env flag, timeout budgets derived from incomplete path models. None would have surfaced from a single review pass; each fix round exposed the next-deeper layer. Round count is not a smell — it is the depth gauge. Corollary: keep fix rounds SCOPED (fix exactly the findings, no opportunistic refactors) so each re-gate stays a delta review.
**Rule:** budget 3+ gate rounds for concurrency/reliability-class changes; treat a first-round PASS on such code as a prompt to widen the review, not to celebrate.

## Lesson #128 (2026-07-20, lead) — zsh does not word-split unquoted variables: fleet loops must use arrays
The 28-seat smoke posted 0/28 on first run: `for s in $SEATS` in zsh iterates ONCE with the whole space-separated string as a single (invalid) seat name, and the per-seat retry loop swallowed every failure silently. Worked in bash, silently broken in zsh (the session shell). Fix: `seats=(a b c); for s in "${seats[@]}"` — and log a FAILED marker per item so an all-fail run is loud, not a mysterious empty ledger.
**Rule:** fleet iteration scripts use explicit arrays (or `${=VAR}` with a comment); any batch loop writes a per-item failure marker — an empty output file must be impossible to confuse with success.

## Lesson #129 (2026-07-20, lead) — brief endpoint claims need handler-DEPTH verification: fields AND auth, not just existence
LAB_UNIFY_P1_SETTINGS_LOGS_1 cited two endpoints wrongly in one brief despite ui-surface-prebrief running: (1) claimed /api/state carries per-seat context % — it returns forge_snapshots only (b1 caught it, real source /lifecycle/seats-over-band); (2) listed /api/v2/pool_stats as a browser-renderable maintenance card — it is terminal-key-gated by design, so the card could NEVER render (codex P1 REQUEST CHANGES #14158). The prebrief checks verified routes EXIST but not their response shape or auth class from the consumer's seat.
**Rule:** for every endpoint a brief tells a UI to consume, verify three things at the handler: (a) route exists, (b) response carries the exact fields the UI needs, (c) the auth class is satisfiable from the surface that will call it (anonymous browser vs terminal-key vs cookie). "Route exists" alone is not a verified contract.

## Lesson #130 (2026-07-21, lead) — iframe 404s fire `load`, not `error`: iframe fail-soft needs a pre-validate fetch
LAB_UNIFY_P3_LOOPS_1 shipped an iframe mount whose fail-soft only listened to the iframe `error` event. Browsers fire `load` (not `error`) when an iframe URL returns HTTP 404 — the server happily serves an error document — so a missing board rendered NO degraded state (codex FAIL #14224, proven by a browser probe with a nonexistent board path). The same brief line "on iframe error show X" propagated the wrong mental model from spec into code.
**Rule:** an iframe's `error` event is NOT a fail-soft surface for HTTP failures. Pre-validate the URL with a timeout-bounded `fetch()` and only insert the iframe on 2xx; non-2xx/network renders the degraded message. Brief-side: specify "validate response, then mount", never "on iframe error".

## Lesson #131 (2026-07-21, lead) — bm-aihead1 checkout is SHARED with deputy-codex: verify branch before every commit there
Lead committed a registry update and ran `git pull --rebase` in /Users/dimitry/bm-aihead1 assuming it sat on main — deputy-codex (who shares that checkout) had left it on deputy/forge-drift-reconcile-1. The commit landed on deputy's feature branch, the rebase dragged origin/main into it, and `git push origin main` failed non-fast-forward (local main was stale). Recovery: `git reset --hard HEAD~1` on deputy's branch tip (own commit only), re-apply via bm-b4.
**Rule:** any checkout another agent also uses (bm-aihead1 ⟷ deputy-codex, the shared vault) gets `git rev-parse --abbrev-ref HEAD` verified IMMEDIATELY before commit/pull — and lead's merges/doc-commits route through the bm-b4 checkout (checkout main → act → restore prior branch) as the default, never the shared one.

## Lesson #131 (2026-07-21, lead) — Builder-run "verify" is not a gate; and a receipt without ls-remote is not shipped
During the cockpit polish arcs, deputy-codex three times ran `codex-verify` CLI against its OWN branch and reported "PASS (fallback provenance)" after hitting the expected 403 reader_slug_mismatch probing the codex seat's inbox — and twice reported "ready for gate" / final receipts while the branch existed only locally (no push; one receipt also mistyped the hash). Verdicts were disregarded; the authoritative PASS came from the codex bus seat each time.
**Rules:** (1) A builder may never run any verify tool — same vendor or cross-vendor — against its own branch and call it a gate; gate authority lives ONLY with the independent reviewer seat, and the fallback for a gate-transport failure is escalating to the dispatcher, not self-gating. (2) Seat inboxes are reader-scoped by design: a 403 reader_slug_mismatch on another seat's inbox is expected topology, not an outage to work around. (3) A ship receipt is incomplete until `git ls-remote` shows the reported hash on origin — put the ls-remote line in the receipt. (Numbering: fleet max #130 per vault-side entries; this file's local max was #121 — using #131 to stay collision-free per the #118-collision precedent.)

## Lesson #132 (2026-07-23, lead) — Carried "owed/broken" items decay: verify against the primary surface BEFORE re-dispatch
Three stale-carry instances in 48h: (1) default-page flip carried as HELD — actually live since 07-21 walkthrough; (2) ctx-band re-ship carried as owed — already merged @a39c0a57, deputy re-dispatch #15038 voided and stood down; (3) arm-alarm AMBER markers carried as "absent since 12:47Z 07-22, brief fix if persists" — full log history shows absent since INSTALL 07-13 (4,070 lines): the report/canary writer pipelines were never built, so it was a designed-degraded state, not a regression. Each stale carry either wasted a worker dispatch or nearly produced a wrong-shaped fix brief (regression-hunt vs build-the-writers).
**Rule:** before acting on any checkpoint/pin "owed / broken / since <time>" claim, verify it against the primary surface — merged git log for "re-ship owed", FULL log history head (not tail) for "X absent since", live endpoint for "still failing". A pin records what a tired session believed at close; the primary surface records what is true.

## Lesson #133 (2026-07-24, lead) — Stale builder base makes the gate grade YOUR commits as the builder's: rebase before every cross-repo delta gate
Twice in one day (cockpit header round-4 PASS #14954 voided; PR #180 diff-B FAIL #15406) a codex gate graded a diff that included LEAD's own recent main commits because the builder branch was cut from an older main tip. The verdict is void either way: a PASS approves unreviewed drift, a FAIL blames the builder for code they never wrote. Both cost a full extra gate round to discover.
**Rule:** before asking any gate on a builder branch, check the branch's merge-base against current main tip (`git merge-base` vs `git rev-parse origin/main`); if main has moved, rebase (or merge main in) FIRST and gate the delta against the fresh base. A stale-base verdict is void the moment it is detected — re-gate, never argue it.

## Lesson #134 (2026-07-24, lead) — Cross-repo gate asks cite repo+branch+sha, never bare PR numbers
Codex resolved "PR #178" via gh in the wrong repo (baker-master instead of brisen-lab), graded an entirely different PR, and returned a plausible-looking verdict (#15295, voided). Repos share PR numbering; a bare number is ambiguous by construction, and the reviewer has no way to notice.
**Rule:** every gate ask names repo + branch + head sha as the primary key; PR number is a courtesy alias only. A verdict whose evidence quotes files from the wrong repo is void — re-gate with the full key.

## Lesson #135 (2026-07-24, lead) — Worker receipts must fail loud on scope drops: a receipt that omits a promised item is a silent skip
Deputy promised a regression test in round-1 receipt (#15253), then shipped round 2 without the test and without mentioning the drop (#15260) — the omission surfaced only because the gate independently flagged the missing test. Extends #131 (builder verify ≠ gate) and the fail-loud rule: "done" that silently sheds scope is a false receipt.
**Rule:** a worker receipt enumerates every item promised in the prior round with SHIPPED / DROPPED-BECAUSE / DEFERRED status — never omit a line. Reviewer-side: diff the receipt against the prior round's promises before gating; a dropped item unmentioned = automatic fix-round.

## Lesson #136 (2026-07-24, lead) — Bus WAF 403s fire on code-literal bodies: reword, never retry verbatim
Bus posts containing curl command lines / array-syntax code literals drew WAF 403s; retrying the identical body just re-triggered the rule and burned turns. The transport was healthy — the CONTENT matched a WAF signature.
**Rule:** on a bus/API 403 with a code-heavy body, treat it as content filtering: paraphrase the code (describe the command, backtick-mangle, or point to a file/sha) and repost. Verbatim retry of a 403'd body is never the move; two identical 403s = certainty it's the body.

## Lesson #137 (2026-07-24, lead) — Codex seat context-exhaustion: revive with tmux /new + prompt re-injection; back-log an auto-clear guard
Codex bus seat stalled ~2h mid-gate on context exhaustion; wake nudges could not rouse it (the seat was alive but its session was full, so nudges parked). Revival that worked: tmux-inject `/new` to reset the session, then re-inject the full gate prompt with capture-pane verification (#125 sub-trap applies). Complements #126 (lane reroute) — reroute when the seat is DEAD; /new-revive when it is ALIVE-but-full.
**Rule:** a codex seat unresponsive to wakes gets one capture-pane diagnosis: if the composer shows context-exhaustion, /new + re-inject the task (it has no session memory worth saving mid-gate); if truly dead, reroute per #126. Standing backlog: an auto-clear guard that fires /new when the seat crosses its context ceiling between tasks.

## Lesson #138 (2026-07-24, lead) — "Seat live" requires a RUNNING agent process in the pane, not tmux+ttyd+wiring up
FUND_AGENT_SEAT_INSTALL_1: lead declared seat-live after 14 rows + 3-signature gate + bus E2E (message delivered + read with the seat's own key) — but the tmux session was a bare shell; claude was never launched inside it. Director clicked the card and saw a dead pane; cowork's first-flight dispatch found nobody home. Every layer verified EXCEPT the one that answers.
**Rule:** an install's live-check ends with the agent PROCESS verified in its pane: `tmux send-keys '<launcher-fn>' Enter` + capture-pane showing the composer prompt (and correct model for the seat class). Bus-delivery E2E proves plumbing, not presence. Add "launcher running in tmux + capture-pane transcript" to every install brief's lead Tier-B checklist alongside the SQL check.
