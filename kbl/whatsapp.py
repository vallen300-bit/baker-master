...
def send_director_alert(message: str, kind: str | None = None) -> bool:
    ...
    (BAKER_WA_DIRECTOR_FILTER_1). Callers passing infra-class alerts (e.g.
    "Anthropic circuit opened", "KBL cost cap reached") should leave `kind`
    as None — the chokepoint will block + audit to baker_actions, restoring
    signal-to-noise on Director's primary alert channel."""
    try:
