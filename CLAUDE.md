...
| 26 | Mar 18 | **20+ commits, largest session. Alert noise + intelligence + agent tools.** ALERT-DEDUP-3 (universal title dedup). Commitment checker/Whoop/VIP gap killed. Obligations 391→77. Alerts 198→95. B1: conversation memory in Qdrant. B3: decision injection in specialists. F1: compounding risk detector (2h). F2: news-counterparty matching. F5: weekly intelligence digest (Sun 18:00). A7: query_baker_data tool (Haiku SQL). A1: create_deadline tool. C2 partial: contact silence detection. Master Backlog v1 (48 items). Code Brisen: desktop alert triage, mobile polish, pipeline dedup, iOS Shortcuts, real-time push alerts (SSE + notifications + T1 sound). |
