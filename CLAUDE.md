...
| 26 | Mar 18 | **25 commits, largest session.** Alert noise: DEDUP-3, commitment/Whoop/VIP killed, 198→95 alerts, 391→77 obligations. Intelligence: B1 conversation memory in Qdrant, B3 decision injection, F1 compounding risk detector (2h), F2 news-counterparty matching, F5 weekly digest (Sun 18:00), C2 contact silence detection, morning narrative cooling warnings. Agent: A7 query_baker_data, A1 create_deadline, A3 create_calendar_event (16 tools total). Strategy: Master Backlog v1 (48 items, 8 domains). Code Brisen: desktop alert triage, mobile polish, pipeline dedup, iOS Shortcuts (/api/scan/quick), real-time push alerts (SSE + notifications + T1 sound), mobile alerts view (swipe dismiss + filter chips). |
