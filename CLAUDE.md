...
| 26 | Mar 18 | **14 commits. Alert noise elimination + intelligence features.** ALERT-DEDUP-3 (universal title dedup). Commitment checker disabled (625 migrated). Soft obligation auto-dismiss (391→77). Email intelligence source_id. Alert cleanup 198→95. Whoop killed (Director decision). B1: conversation memory embedded to Qdrant. B3: past decisions injected into specialist prompts. F1: compounding risk detector (2h cycle, per-matter scoring). F2: RSS news-counterparty matching (dynamic matter/VIP context). VIP gap monitoring disabled (all contacts equal). Master Backlog v1 (48 items, 8 domains). Code Brisen: desktop alert triage UI, mobile polish, pipeline source_id dedup, iOS Shortcuts (/api/scan/quick). |
