...
async def get_sentinel_health():
    ...
    sentinels = []
    summary = {"healthy": 0, "degraded": 0, "down": 0, "stale": 0, "disabled": 0, "unknown": 0}
    for r in rows:
...
# COCKPIT_REFERENCE_DESK_1 Fix 4: in-process 60s cache of the full morning-brief
# payload. The endpoint aggregates ~12 sequential reads + a Haiku narrative and
# measured ~12s cold — a blank first screen. Warm calls within 60s return the last
# payload instantly (no cross-instance coordination needed; a stale-by-<=60s brief
# is fine for a landing view). Cold path stays instrumented (see _timings) so the
# real offender ranking is readable from prod logs for a timing-gated follow-up.
_MORNING_BRIEF_CACHE = {"ts": 0.0, "payload": None}
_MORNING_BRIEF_CACHE_TTL = 60.0  # seconds
...
async def get_morning_brief():
    ...
    Narrative generated by Haiku, cached 30 min. Full payload cached 60s in-process.
    """
    # Warm-cache short-circuit (COCKPIT_REFERENCE_DESK_1 Fix 4).
    _cache_now = time.monotonic()
    if (_MORNING_BRIEF_CACHE["payload"] is not None
            and (_cache_now - _MORNING_BRIEF_CACHE["ts"]) < _MORNING_BRIEF_CACHE_TTL):
        return _MORNING_BRIEF_CACHE["payload"]
    ...
    try:
        ...
        _timings["db"] = round(time.monotonic() - _t0, 3)
        ...
        _timings["narrative"] = round(time.monotonic() - _t0 - _timings.get("db", 0), 3)
        ...
        _timings["total"] = round(time.monotonic() - _t0, 3)
        _timings["rest"] = round(_timings["total"] - _timings.get("db", 0) - _timings.get("narrative", 0), 3)
        # COCKPIT_REFERENCE_DESK_1 Fix 4 diagnose instrument: the section timings are
        # logged so the true cold offender (db fan-out vs narrative vs rest) is
        # readable from prod logs — do not guess a concurrency refactor before this.
        logger.info("morning-brief cold timing (s): %s", _timings)
        ...
        result = {
            ...
            # COCKPIT_REFERENCE_DESK_1 Fix 4: when this payload was assembled (UTC).
            "as_of": datetime.now(timezone.utc).isoformat(),
        }
        _MORNING_BRIEF_CACHE["ts"] = time.monotonic()
        _MORNING_BRIEF_CACHE["payload"] = result
        return result
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"GET /api/dashboard/morning-brief failed: {e}")
        # Do NOT cache the degraded payload — next call retries the full build.
        return {
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "fallback_mode": None,
                    # COCKPIT_REFERENCE_DESK_1 Fix 4: assembly time (UTC) for the as-of stamp.
                    "as_of": datetime.now(timezone.utc).isoformat(),
                }
            except HTTPException:
                raise
...
# AO_FLIGHT_RELATIONSHIP_1 Fix 3 (Director directive 2026-07-09): the cockpit AO
# Dashboard is discarded. Its two live assets — pm_project_state slug 'ao_pm' and the
# vip_contacts orbit — plus the (now honest) comms-gap wiring were moved to the
# AO-OSK-001 flight page (/flight/AO-OSK-001). The rest of the old tab was dead or
# lying: view_files read data/ao_pm/*.md (deleted 2026-04-22, f3bbd16b); the comms-gap
# defaulted fake-GREEN on a silent double query failure (wrong WA chat key + a
# non-existent sent_at column); decisions / comms_log / pending_insights were empty.
# Helpers _load_ao_view_files / _get_ao_orbit_names removed (nothing else imported them).
# The AO investment headline (was AO_INVESTMENT_TOTAL = "EUR 66.5M" as-of 2026-06-01) is
# AO-Desk-owned — it belongs on the flight page's money_kpis at the desk's discretion,
# not as an orphan constant here.


@app.get("/api/dashboard/ao")
async def get_ao_dashboard():
    """DISCARDED — moved to the AO-OSK-001 flight page. Returns 410 Gone (a signpost,
    not a 404 which would read as a bug). AO_FLIGHT_RELATIONSHIP_1, Director 2026-07-09."""
    raise HTTPException(
        status_code=410,
        detail="AO dashboard moved to /flight/AO-OSK-001",
    )
