...
while : ; do
  ...
  if OUT="$(curl -sS --max-time 20 -w '\n%{http_code}' -H "X-Terminal-Key: $KEY" \
      "https://brisen-lab.onrender.com/msg/codex-arch?limit=${LIMIT}&unread=true" 2>/dev/null)"; then
    HTTP_CODE="${OUT##*$'\n'}"
    RESPONSE="${OUT%$'\n'*}"
  else
    HTTP_CODE="000"
...
if [[ "$HTTP_CODE" == "000" ]]; then
  echo "ERROR: brisen-lab unreachable (GET /msg/codex-arch) after $((_RETRY_MAX + 1)) attempt(s)."
  echo "ERROR: brisen-lab unreachable (GET /msg/codex-arch) after $((_RETRY_MAX + 1)) attempt(s)." >&2
  exit 4
fi
...
try:
  ...
  echo "ERROR: brisen-lab GET /msg/codex-arch returned HTTP ${HTTP_CODE}${_DETAIL:+ (${_DETAIL})} — NOT empty; bus busy/degraded, retry shortly."
  echo "ERROR: brisen-lab GET /msg/codex-arch returned HTTP ${HTTP_CODE} ${_DETAIL} — NOT empty." >&2
  exit 4
fi
...
if not msgs:
    if complete is False:
        print("codex-arch inbox: PARTIAL read (complete=false) — NOT empty; widen limit or drain cursor.")
        sys.exit(5)
