...
# started signal (codex's first non-ack reply => that dispatch is `started`). Omit for a
# non-reply post (byte-identical to before).
PARENT="${4:-}"
...
if [[ -n "$PARENT" ]]; then
  BAKER_ROLE=codex exec "$BUS_POST" "$RECIPIENT" "$BODY" "$TOPIC" --parent "$PARENT"
else
  BAKER_ROLE=codex exec "$BUS_POST" "$RECIPIENT" "$BODY" "$TOPIC"
fi
