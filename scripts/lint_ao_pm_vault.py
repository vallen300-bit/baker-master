...
- Interactions files missing 4 required timestamps
  (source_at/ingest_at/recall_at/decision_at).
...
logger = logging.getLogger("lint_ao_pm_vault")
    ...
    try:
        ...
        try:
            ...
            for row_id, rule in cur.fetchall():
                violations.append(
                    f"Stale correction #{row_id} (ao_pm): {(rule or '')[:80]}… — retired-candidate"
                )
            cur.close()
        except Exception as e:
            try:
