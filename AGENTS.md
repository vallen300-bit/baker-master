...
Canonical: `~/baker-vault/_ops/processes/director-comm-rules.md` + global Tier-0 `/Users/dimitry/.Codex/dropbox-tier0.md` §"DIRECTOR ROLE / AGENT ROLE".
...
- End multi-option replies with explicit `Recommendation: X — why` (hook-enforced via `~/.Codex/hooks/recommendation-check.sh`; subsumes Rule 2's recommendation clause).
...
> 3. Invoke the Read tool on `~/.Codex/projects/-Users-dimitry-Vallen-Dropbox-Dimitry-vallen-Baker-Project/memory/MEMORY.md` (canonical Baker memory — V12-V14 handovers, Cortex roadmap, in-flight briefs, accumulated lessons).
>
> **No "by inspection".** Actually invoke the Read tool — do not improvise from this AGENTS.md alone.
>
...
> 1. *Global rules + Tier 0 portfolio context (`/Users/dimitry/.Codex/AGENTS.md` + imported `dropbox-tier0.md`) are harness-auto-loaded — do NOT Read again. Sanity check: confirm Rule 1 ("Director is non-technical") is visible in context; if missing, fall back to Read on `/Users/dimitry/.Codex/AGENTS.md`.*
...
> 4. Invoke the Read tool on `~/.Codex/skills/laconic/SKILL.md` (canonical Director-facing register; default per dropbox-tier0.md Rule 6, ratified 2026-05-25).
>
...
> | matter context, desk shadow-org, AO/MOVIE/Hagenauer/Eastdil/Heidenauer disambig, Cortex history, Todoist API, BB-Desk, active roadmap pointers, strategic principles | `~/.Codex/projects/-Users-dimitry-Vallen-Dropbox-Dimitry-vallen-Baker-Project/memory/MEMORY.md` (curated index — engineer/architect roles lazy-load this; Director-ratified 2026-05-23 PM2 to keep AH slim at session start) |
>
...
>
> Block applies when cwd path is `/Users/dimitry/bm-aihead2` OR a Cowork-spawned worktree under it (`bm-aihead2/.Codex/worktrees/<name>`). Pre-2026-05-10 the check was strict basename only, which broke when Cowork forced worktree-mode and spawned sessions under `.Codex/worktrees/`. B-code sessions follow the B-code block above; AH1 sessions follow the AH1 block below.
...
voyage-3, 1024d), Codex Opus via Anthropic API, vanilla JS frontend, Render
...
- `tools/` → MCP tools (24 tools per `.Codex/docs/baker-mcp-api.md`)
...
- **Codex auto-memory** (this repo, this CLI session): `~/.Codex/projects/<slug>/memory/`.
  Hot cache = this AGENTS.md. Index = `MEMORY.md` (first 200 lines load every session).
...
- **Baker MCP API patterns:** `.Codex/docs/baker-mcp-api.md`
- **Critical IDs** (workspaces, lists, contacts): `.Codex/docs/critical-ids.md`
- **Path-scoped rules:** `.Codex/rules/`
- **Specialized agents:** `~/.Codex/agents/` (user-global — not in repo per HARNESS_SUBAGENT_MIGRATION_1)
